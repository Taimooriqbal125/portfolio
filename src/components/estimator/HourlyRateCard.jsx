...
export default function HourlyRateCard({ data = HOURLY_CARD }) {
  return (
    <aside className="flex flex-col rounded-3xl border border-emerald-500/30 bg-emerald-950/40 p-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-xs text-emerald-400">{data.tagline}</p>
      ...
      <a href={data.href} className="mt-auto block pt-5">
        <span className="block rounded-xl bg-emerald-600 py-2.5 text-center text-sm font-bold text-white transition-colors hover:bg-emerald-500">
          {data.cta}
        </span>
      </a>
    </aside>
